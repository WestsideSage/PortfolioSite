...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    const response = await fetch(`${SPOTIFY_API_BASE}/me/player/seek?position_ms=${position_ms}`, {
      method: 'PUT',
      headers: {
    ...
    if (response.status === 204 || response.status === 200) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Failed to seek. Make sure Spotify is active and you have Premium.',
        }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
