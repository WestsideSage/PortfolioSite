...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    const response = await fetch(`${SPOTIFY_API_BASE}/me/player/shuffle?state=${state}`, {
      method: 'PUT',
      headers: {
    ...
    if (response.status === 204 || response.status === 200) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Failed to toggle shuffle. Make sure Spotify is active and you have Premium.',
        }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
